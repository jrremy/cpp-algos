...
void print(int q[])
{
    ...
    // Go through q[], which tells us where the queen is placed in each column
    for (int i = 0; i < 8; i++)
    {
        // If the box is white (points to wb) then point it to a white queen (wq)
        // Else, point it to a black queen (bq)
        if ((i + q[i]) % 2 == 0)
            board[i][q[i]] = &wq;
        else
            board[i][q[i]] = &bq;
...
int main()
{
    ...
    {        // NEXT COL:
        ...
        if (c > 7)
        {             // If you have passed the last column, a solution is found
        ...
        {           // NEXT ROW:
            ...
            if (q[c] > 7)
                c--; // If you have passed the end of the row, backtrack
            else     // Otherwise, call the ok function.
                if (ok(q, c))
                    break; // If ok (returns true), a queen is filled in the row
        }
    }
    return 0;
}