...
{
    vector<int> t[3];
    // The program works for both even and odd values of n
    int n;
    cout << "Please enter the number of rings to move: ";
    cin >> n;
    cout << endl;
    // The initial value of to depends on whether n is odd or even
    // To = 2 if n is even, 1 if n is odd
    ...
    { // while t[1] does not contain all of the rings
        ...
        if (candidate > t[(n % 2 == 0 ? (from + 2) : (from + 1)) % 3].back())
        {
            to = (n % 2 == 0 ? (from + 1) : (from + 2)) % 3;
        }
        else
        {
            to = (n % 2 == 0 ? (from + 2) : (from + 1)) % 3;
        }
    }
    return 0;
}